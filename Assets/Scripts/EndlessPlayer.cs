...
public class EndlessPlayer : MonoBehaviour
{
    ...
    public TMP_Text timeSurvivedSummary;
    public TMP_Text killsSummary;
    ...
    [SerializeField] private EndlessPauseMenu pause;
    ...
    public EndlessEnemySpawner spawnEnemy;
    private List<EndlessEnemy> enemies = new List<EndlessEnemy>();
    ...
    void Update()
    {
        ...
        if (isDead)
        {
            timer.text = " ";
            kills.text = " ";
            killsSummary.text = "Total kills: " + kill;
            TimeSpan timeSpan = TimeSpan.FromSeconds(timeSurvived);
            timeSurvivedSummary.text = string.Format("Time survived: {0:D2}:{1:D2}:{2:D2}", timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds);
            return;
        }
    ...
    public void useSkill()
    {
        canUseSkill = false;
        timerCooldown = skillCooldown;
        List<EndlessEnemy> enemiesToRemove = new List<EndlessEnemy>();
        ...
        foreach (EndlessEnemy enemy in enemiesToRemove)
        {
            enemy.die();
    ...
    {
        TimeSpan timeSpan = TimeSpan.FromSeconds(timeSurvived);
        timer.text = string.Format("Time survived: {0:D2}:{1:D2}:{2:D2}", timeSpan.Hours, timeSpan.Minutes, timeSpan.Seconds);
        kills.text = "Kills: " + kill;
    }
    ...
    public void RegisterEnemy(EndlessEnemy enemy)
    {
        enemies.Add(enemy);
    }
