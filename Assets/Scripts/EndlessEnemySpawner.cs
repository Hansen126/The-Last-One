...
public class EndlessEnemySpawner : MonoBehaviour
{
    ...
    private IEnumerator SpawnEnemyRoutine()
    {
        while (player.isDead == false)
        {
            ...
            if (player.timeSurvived > 270)
            {
                ...
                float elapsedTime = player.timeSurvived - 270;

                if (elapsedTime > 0 && (int)(elapsedTime / 30) > (int)((elapsedTime - Time.deltaTime) / 30))
                {
                    spawnInterval = Mathf.Max(0.1f, spawnInterval - 0.1f);
                }

                if (elapsedTime > 0 && (int)(elapsedTime / 90) > (int)((elapsedTime - Time.deltaTime) / 90))
                {
                    spawnInterval *= 2.0f;
                }
            }
    ...
    private void SpawnEnemy()
    {
        ...
        EndlessPlayer playerScript = GameObject.FindGameObjectWithTag("Player").GetComponent<EndlessPlayer>();
        if (playerScript != null)
        {
            EndlessEnemy enemy = enemyObject.GetComponent<EndlessEnemy>();
